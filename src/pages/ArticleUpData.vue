...
import { getTags, getArticleDetail } from "@/api/index";
...
function handleValidateClick() {
  ...
  if (!formValue.title) {
    window.$message.error("标题不能为空格");
    return;
  }
  formRef.value.validate(async (errors) => {
    if (!errors) {
      let res = await service.post("/article/update/" + id.value, {
        title: formValue.title,
        description: formValue.description,
        tag_id: formValue.tag_id,
...
async function initArticle() {
  ...
  formValue.title = result.data.title;
  formValue.description = result.data.description;
  formValue.tag_id = result.data.tag_id;
  formValue.content = result.data.content;
  formValue.author_id = result.data.author_id;
}
